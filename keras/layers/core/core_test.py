...
class CoreLayersTest(keras_parameterized.TestCase):
  ...
  def test_dense_layer_ragged_tensor(self):
    layer = keras.layers.Dense(2, kernel_initializer='ones')

    # a.shape = [2, None, 2]; a.ragged_rank=1
    a = tf.ragged.constant([[[1, 2], [3, 4], [5, 6]], [[7, 8]]],
                           ragged_rank=1)
    self.assertAllEqual(layer(a), [[[3, 3], [7, 7], [11, 11]], [[15, 15]]])
  ...
  def test_dense_layer_unsupported_ragged_tensor_error(self):
    ...
    with self.assertRaisesRegex(
        ...
        r'be defined. Found None. Full input shape received: \(2, None\)'):
      layer(tf.ragged.constant([[1, 2], [3, 4, 5]]))
    with self.assertRaisesRegex(
        ValueError, 'Dense layer only supports RaggedTensors when the innermost'
        r' dimension is non-ragged.  Received: inputs.shape=\(2, None\)'):
